.. dbfp_anomaly documentation master file, created by
   sphinx-quickstart on Thu Sep  5 21:34:50 2019.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to kuberspatiotemporal
==============================

A package to experiment with probabilistic,
heterogeneous mixture models for risk analysis.
Based on `(Kimura et al. 2011)`_ by DeepMind's `Arnaud Doucet`_ and
`(Krueger et al. 2018)`_, and maybe also `(Neal and Hinton 1998)`_.


.. toctree::
   :maxdepth: 2
   :caption: Contents:

   math_fundamentals
   classical_batch
   discussion
   implementation
   api/modules

.. _(Neal and Hinton 1998): https://www.cs.toronto.edu/~hinton/absps/emk.pdf
.. _(Kimura et al. 2011): https://link.springer.com/article/10.1007/s10044-011-0256-4
.. _Arnaud Doucet: https://www.turing.ac.uk/people/researchers/arnaud-doucet
.. _(Krueger et al. 2018): https://arxiv.org/pdf/1801.06296.pdf

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
